...
from intermine_boot.commands import assert_docker
...
def main(options, env, minecompose_path):
    ...
    (tomcat_container, tomcat_status) = create_tomcat_container(client, tomcat_image)
    ...
    (postgres_container, postgres_status) = create_postgres_container(client, postgres_image, options, env, network_name=DOCKER_NETWORK_NAME)

    if not (tomcat_status and solr_status and postgres_status):
        click.echo('Error occurred when starting containers', err=True)
        sys.exit(1)

    # TODO docker_intermine_gradle: add build.sh DB initialisation code to postgres.Dockerfile
